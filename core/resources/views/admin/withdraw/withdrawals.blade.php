        ...
        @if (request()->routeIs('admin.withdraw.log'))
            <div class="col-xl-4 col-sm-6 mb-30">
                <div class="widget-two box--shadow2 b-radius--5 bg--success">
                    <div class="widget-two__content">
                        <h2 class="text-white">{{ __($general->cur_sym) }}{{ showAmount($successful, $general->currency) }}
                        </h2>
            ...
            <div class="col-xl-4 col-sm-6 mb-30">
                <div class="widget-two box--shadow2 b-radius--5 bg--6">
                    <div class="widget-two__content">
                        <h2 class="text-white">{{ __($general->cur_sym) }}{{ showAmount($pending, $general->currency) }}</h2>
            ...
            <div class="col-xl-4 col-sm-6 mb-30">
                <div class="widget-two box--shadow2 b-radius--5 bg--pink">
                    <div class="widget-two__content">
                        <h2 class="text-white">{{ __($general->cur_sym) }}{{ showAmount($rejected, $general->currency) }}
                        </h2>
                                ...
                                <tr>
                                    ...
                                    <td data-label="@lang('User')">
                                        ...
                                        @elseif($withdraw->user_type == 'AGENT')
                                            ...
                                            <span class="small"> <a
                                                    href="{{ route('admin.users.detail', $withdraw->user_id) }}"><span>@</span>{{ $withdraw->agent->username }}</a>
                                            </span>
                                        @else
                                            <span class="font-weight-bold">{{ $withdraw->merchant->fullname }}</span><br>
                                            <span class="small"> <a
                                                    href="{{ route('admin.users.detail', $withdraw->user_id) }}"><span>@</span>{{ $withdraw->merchant->username }}</a>
                                            </span>
                                        @endif
                                    ...
                                    <td data-label="@lang('Amount')">
                                        {{ showAmount($withdraw->amount, $general->currency) }}
                                        {{ __($withdraw->curr->currency_code) }} - <span class="text-danger"
                                            data-toggle="tooltip"
                                            data-original-title="@lang('charge')">{{ showAmount($withdraw->charge, $general->currency) }}
                                        </span>
                                        <br>
                                        <strong data-toggle="tooltip" data-original-title="@lang('Amount after charge')">
                                            {{ showAmount($withdraw->amount - $withdraw->charge, $general->currency) }}
                                            {{ __($withdraw->curr->currency_code) }}
                                        </strong>
...
@push('breadcrumb-plugins')
    @if (!request()->routeIs('admin.users.withdrawals') &&
        ...
        !request()->routeIs('admin.agent.withdrawals') &&
        !request()->routeIs('admin.agent.withdrawals.method') &&
        !request()->routeIs('admin.merchant.withdrawals') &&
        !request()->routeIs('admin.merchant.withdrawals.method'))
        <form
